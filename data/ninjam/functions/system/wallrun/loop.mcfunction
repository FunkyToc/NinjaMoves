execute if entity @s[predicate=ninjam:sprinting,tag=!ninjam_wall3] if block ~ ~-1 ~ #ninjam:runnable unless entity @e[type=minecraft:area_effect_cloud,tag=ninjam_running,distance=..0.5] run function ninjam:system/wallrun/check
