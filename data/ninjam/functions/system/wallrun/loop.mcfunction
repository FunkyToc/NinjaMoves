...
execute as @e[type=area_effect_cloud,tag=ninjam.running,scores={ninjam.Jump=0}] at @s run function ninjam:system/wallrun/despawn
scoreboard players remove @e[type=area_effect_cloud,tag=ninjam.running,scores={ninjam.Jump=1..},distance=..1] ninjam.Jump 1
